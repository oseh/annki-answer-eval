...
---------------------------------
Evaluates the learner's typed answer with OpenAI and shows
immediate AI feedback in a pop-up.

Install
=======
1. Place this file in a new folder inside Anki's add-on directory, e.g. `answer_eval`.
2. Add a `manifest.json` next to it:
   {
     "package": "answer_eval",
...
Tested on Anki 24.11. Compatible with Python 3.9+.
"""
...
# Keep dialogs alive to prevent garbage collection
...
# ---------------------------------------------------------------------------
# OpenAI REST call (worker thread)
# ---------------------------------------------------------------------------
...
class FeedbackDialog(QDialog):
    def __init__(self, mw, score, feedback, expected, ai_ease, ai_label, ai_color, mnemonic, parent=None, card=None):
        ...
        """
        # Add AI-generated mnemonic to popup
        if mnemonic:
        ...
        # Set stylesheet for checked/unchecked state
        self.setStyleSheet('''
...
''')
    ...
    def select_ease(self, ease):
        ...
        # No need to update styles manually; stylesheet handles it
...
# ---------------------------------------------------------------------------
# Hook callback: after answering a card
# ---------------------------------------------------------------------------

def _on_answer(reviewer, card, ease):
    try:
        # Robustness: check for typed answer support
        ...
        # Run grading and mnemonic generation in a background thread
